...
`last_ts` is the newest detection _in the cluster_ and `quiet_after` is `quiet_hours`
...
it asserts is that _a satellite still reports heat_, not that the fire is burning:
...
new fire must not deliver it invisibly. The reset sits _after_ the
...
`window_hours` (8760) is how far back events are _built_; `retention_days` (400) is how
long detections are _kept_. Retention must exceed the largest view range or the oldest
...
5. **FIRMS `days` is 1–5 only** and `start_date` counts _forward_ from itself. Errors
   are HTTP 400 with an English sentence, not JSON; a valid query with no fires is
   HTTP 200 with only the CSV header. The guard checks status _and_ body shape.
...
8. **In the map, pixel and metre radii are different encodings.** `L.circleMarker`
   radius is screen pixels (fire _intensity_, fixed at all zooms); `L.circle` radius
   is metres (fire _footprint_, scales with zoom). Conflating them made markers look
...
12. **Never point an ngrok `file://` tunnel at a path containing a space.**
    ...
    in _Application Support_. A literal space fails too — the agent API rejects the
    URL outright.
...
104` ("not yet ready"), so `agent_up()` returning True is not the same as
    ready. Worse, a POST that fails this way _keeps the name_: afterwards it 404s
...
14. **`L.Polygon.getCenter()` throws until the layer is on a map** ("Must add
    ...
    its layer group _before_ it builds the area label, not after.
...
17. **Lowering `quiet_hours` _increases_ SMS volume.** The intuition is that a
    ...
    **It also has a hard floor: the slowest feed's delivery latency.** Sentinel-3
    ...
    burning _again_ when it had never been reported once. Replaying all stored
...
18. **A database carries a place, and only says so if it was created after the
    stamp.** `store` writes `meta.place_id` when it first sees an _empty_ database.
    ...
    was tried and dropped - a fork usually starts from a _neighbouring_
    municipality, whose 2 km bands overlap, and 35 of Zavidovići's 198 detections
    fall inside Kakanj's clip. So `place` reports the out-of-scope _count_, which
...
no dependency. Note the WMS host is the _same GeoServer_ `sources.py` already
...
flame - showed _nothing_. At this municipality's fire sizes the plume is the
...
(`imagery.py`). Sentinel Hub takes its credential as an _instance id in the URL
path_, so a browser-side layer would write a quota-bearing credential into
...
proxy through. The catalogue half _is_ keyless (CDSE OData), which is why the
...
_Meteosat FCI_ one ("cannot be made directly from FCI because it lacks a Green
...
20. **`rgb_truecolour` and `vis06_hrfi` need _two_ separate guards, and
    ...
    But those night frames are _useless_, and in two different disguises:
    ...
    **Only the geostationary visible layers carry `needsSun`.** The GIBS layers
    ...
    run on the _live_ branch too - the default view is the live edge, so a
...
25. **The Sentinel-2 render is requested in EPSG:3857, not 4326.**
    `L.imageOverlay` stretches its image linearly between two _projected_
...
28. **Imagery exclusivity is per _group_, in code, not by Leaflet.** Base
...
The three fire feeds report _detections_ - something is already hot. `firedanger.py`
...
separate "gap too large to bridge" path - every run replays the _entire_
fetched window from the standard startup values (FFMC=85, DMC=6, DC=15). One
Open-Meteo call (`api.open-meteo.com/v1/forecast`, keyless) _asks_ for
...
Open-Meteo's 16-day allowance because the _weather_ forecast under it, not the
...
on a network with no IPv6 egress, FIRMS is the _only_ feed that breaks, with
...
Caddy for TLS. `serve` polls _and_ serves in one process on purpose - a host serving
...
independent of the desktop notification — `mark_notified` fires if _either_
...
- The API key comes from `HTTPSMS_API_KEY` or the macOS Keychain (service
  ...
  <https://httpsms.com/settings>, not the _phone_ API key from
  ...
  wrong key fails late and quietly: `ready()` only checks that a key is _present_, so
  ...
  from httpSMS means _accepted_, not sent - the phone still has to be awake, online and
...
This repository _is_ a running instance, not just the source of one. `.github/workflows/`
...
workflow logs the recipient _count_, never the numbers, since GitHub masks an exact secret
...
_GitHub Actions_ and not a branch.
...
what they resolved. The repository _variable_ `FIREWATCH_PUBLIC_URL` overrides it, for
a custom domain.
...
- `~/Library/Caches/FireWatch/public/` — the only directory ever exposed publicly.
  Rewritten from the snapshot every cycle, so losing it costs nothing. It is _not_
...
- **No key is a supported state, not a failure.** `fetch_firms` raises
  `sources.NoCredentials`, the poller records `configured: False` and logs at _info_,
...
- **The Copernicus client resolves the same way**, through `cdse_credentials()`:
  ...
  mistake. `secrets()` adds the raw item _and_ each half separately: the combined
...
- **On macOS the key belongs in the Keychain, not the environment.** `launchd` does not
  ...
  keys resolve the same way, so the agent silently loses FIRMS _and_ SMS while a
...
- **The FIRMS key travels in the URL _path_**, so any `requests` exception carries the
...
Notifications go through `osascript` (attributed to _Script Editor_, click does
...
- Code style follows the existing modules: `from __future__ import annotations`,
  module-level docstring explaining _why_, comments reserved for non-obvious
